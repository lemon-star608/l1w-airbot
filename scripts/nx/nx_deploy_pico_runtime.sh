...
test -r "$DEB"
test -d "$PYBIND"
test -f "$VENDOR_ROOT/include/nlohmann/json.hpp"
test -f "$VENDOR_ROOT/include/nlohmann/json_fwd.hpp"
test -r "$WHEELS/pybind11-2.13.6-py3-none-any.whl"
...
python3 -m pip install --user --no-index --no-deps \
  ../vendor/wheels/pybind11-2.13.6-py3-none-any.whl
